...
def detect_action(user_query: str) -> Dict[str, Any]:
    ...
    if any(pattern in query for pattern in [
        'group by', 'grouped by', 'for each', 'per '
    ]):
        ...
        if group_field:
            ...
            if any(pattern in query for pattern in ['total', 'sum of', 'sum', 'add up']):
                result["aggregation"] = "sum"
            elif any(pattern in query for pattern in ['average', 'avg', 'mean']):
                result["aggregation"] = "avg"
            elif any(pattern in query for pattern in ['count', 'number of', 'how many']):
                result["aggregation"] = "count"
            elif result.get("aggregation") is None:
                result["aggregation"] = "count"
...
def extract_group_by_field(query: str) -> Optional[str]:
    ...
    patterns = [
        r'group(?:ed)?\s+by\s+([a-zA-Z][a-zA-Z0-9_\s]+?)(?:\s+(?:between|from|for|in|where|include|including)\b|$|[,.;])',
        r'for\s+each\s+([a-zA-Z][a-zA-Z0-9_\s]+?)(?:\s+(?:between|from|for|in|where|include|including)\b|$|[,.;])',
        r'per\s+([a-zA-Z][a-zA-Z0-9_\s]+?)(?:\s+(?:between|from|for|in|where|include|including)\b|$|[,.;])',
        r'by\s+([a-zA-Z][a-zA-Z0-9_\s]+?)(?:\s+(?:between|from|for|in|where|include|including)\b|$|[,.;])',
    ]
    ...
    for pattern in patterns:
        ...
        if match:
            ...
            group_phrase = re.sub(r'\s+', ' ', group_phrase)
            if group_phrase:
                return group_phrase
