...
from action_detector import ActionType
...
COLLECTION_FALLBACKS = {
    "signin_records": ["login_activities"],
}
...
def load_collection_data(database_name: str, collection_name: str) -> List[Dict]:
    ...
    try:
        with open(db_path, 'r', encoding='utf-8') as f:
            ...
            # Fallback aliases for empty collections
            for fallback_collection in COLLECTION_FALLBACKS.get(collection_name, []):
                fallback_path = DATABASES_PATH / database_name / f"{fallback_collection}.json"
                if not fallback_path.exists():
                    continue
                try:
                    with open(fallback_path, 'r', encoding='utf-8') as fb:
                        fb_data = json.load(fb)
                        fb_rows = fb_data if isinstance(fb_data, list) else []
                        if fb_rows:
                            return fb_rows
                except Exception:
                    continue
...
def apply_filters(data: List[Dict], filters: Dict[str, Any]) -> List[Dict]:
    ...
    if 'similar_names' in filters:
        ...
        if name_field:
            ...
            # Find similar names (similarity > 0.6)
            similar_groups = {}
            for name in names:
...
def execute_group_by(data: List[Dict], metadata: Dict) -> Dict:
    ...
    group_field_hint = metadata.get("group_by")
    raw_query = (metadata.get("raw_query") or "").lower()

    if not data:
        return {
            "grouped_by": group_field_hint or "field",
    ...
    sample_keys_compact = {_compact(k): k for k in sample_keys}

    def _resolve_field(field_hint: Optional[str], fallback_tokens: List[str]) -> Optional[str]:
        if field_hint:
            hint_compact = _compact(field_hint)
            if hint_compact in sample_keys_compact:
                return sample_keys_compact[hint_compact]
            for k in sample_keys:
                ck = _compact(k)
                if hint_compact and (hint_compact in ck or ck in hint_compact):
                    return k
        ...
        for token in fallback_tokens:
            ...
            for k in sample_keys:
                ck = _compact(k)
                if token_compact in ck:
                    return k
        return None

    group_field = _resolve_field(group_field_hint, [
        'merchantName', 'merchantId', 'customerName', 'customerId',
        'customerEmail', 'merchantEmail', 'status', 'type', 'date'
    ])
    ...
    # Apply aggregation
    aggregation = metadata.get("aggregation") or "count"
    if aggregation not in ["count", "sum", "avg"]:
        aggregation = "count"

    if aggregation == "count":
        if any(token in raw_query for token in ['total', 'sum of', 'sum', 'add up']):
            aggregation = "sum"
        elif any(token in raw_query for token in ['average', 'avg', 'mean']):
            aggregation = "avg"

    metric_hint = None
    metric_candidates = []
    if 'payout' in raw_query:
        metric_candidates.extend(['payoutAmount', 'amount'])
    if any(token in raw_query for token in ['transaction amount', 'purchase amount', 'sales amount', 'total purchase', 'total transaction']):
        metric_candidates.extend(['amount', 'totalAmountPaid', 'transactionAmount', 'totalAmount'])
    if 'amount' in raw_query:
        metric_candidates.extend(['amount', 'totalAmountPaid', 'transactionAmount', 'totalAmount'])
    if 'points' in raw_query:
        metric_candidates.extend(['points', 'numoniPoints', 'rewardPoints'])

    if metric_hint is None and aggregation in ['sum', 'avg']:
        metric_hint = _resolve_field(None, metric_candidates or ['amount', 'totalAmount', 'transactionAmount', 'totalAmountPaid'])
    ...
    for key, records in groups.items():
        if aggregation == "count":
            ...
            if not field:
                ...
                field = numeric_fields[0] if numeric_fields else None
            if field:
                total = 0.0
                for r in records:
                    try:
                        total += float(str(r.get(field, 0)).replace(',', ''))
                    except Exception:
                        pass
                results[key] = total
            else:
