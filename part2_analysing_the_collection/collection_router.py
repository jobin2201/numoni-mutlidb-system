...
def detect_collection(user_query, database_name):
    ...
    # Collection/table explicit mentions in query should have strong priority
    query_compact = re.sub(r"[^a-z0-9]", "", original_query)
    ...
    for collection_name, metadata in collections_metadata.items():
        ...
        if collection_name.lower().endswith("s"):
            base_alias = collection_name.lower()[:-1]
            collection_aliases.add(base_alias)
            collection_aliases.add(base_alias.replace("_", " "))
            collection_aliases.add(base_alias.replace("_", ""))

        explicit_table_hit = False
        for alias in collection_aliases:
            alias = alias.strip()
            if not alias:
                continue
            alias_compact = re.sub(r"[^a-z0-9]", "", alias)
            if alias in original_query or (alias_compact and alias_compact in query_compact):
                alias_word_count = len([w for w in alias.split() if w])
                score += 180 if alias_word_count >= 2 else 120
                explicit_table_hit = True
                reasons.append(f"[OK] Explicit table name match: {alias}")
                break
        
        # CRITICAL RULE: Entity creation queries should use details collections
        if is_creation_query and not is_transaction_query:
            if explicit_table_hit:
                score += 40
                reasons.append("[OK] Explicit table mention overrides generic creation rule")
            elif 'details' in collection_name.lower():
    ...
    top_name, top_info = top_collection
    top_records = int(top_info.get("total_records") or 0)

    if top_records == 0:
        for candidate_name, candidate_info in sorted_collections[1:5]:
            candidate_records = int(candidate_info.get("total_records") or 0)
            if candidate_records <= 0:
                continue
            if candidate_info["score"] >= top_info["score"] - 30:
                top_collection = (candidate_name, candidate_info)
                break
