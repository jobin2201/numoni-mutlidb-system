...
def detect_advanced_filters(user_query: str, current_date: Optional[datetime] = None) -> Dict[str, Any]:
    ...
    filters = {
        ...
        'rank_filters': extract_rank_filters(query_lower),
...
def extract_date_filters(query: str, current_date: datetime) -> Dict[str, Any]:
    ...
    # First, try robust explicit date-range parsing from field_filter (e.g., between/from-to with years)
    try:
        from field_filter import extract_date_range
        explicit_range = extract_date_range(query)
        if explicit_range:
            date_filters['start_date'] = explicit_range[0]
            date_filters['end_date'] = explicit_range[1]
    ...
    # Pattern: "today"
    if 'today' in query:
        date_filters['days_ago'] = 0
        date_filters['start_date'] = day_start
        date_filters['end_date'] = day_end
        date_filters['type'] = 'days_range'

    # Pattern: "yesterday"
    elif 'yesterday' in query:
        y_start = day_start - timedelta(days=1)
        y_end = day_end - timedelta(days=1)
        date_filters['days_ago'] = 1
        date_filters['start_date'] = y_start
        date_filters['end_date'] = y_end
        date_filters['type'] = 'days_range'
    ...
    # Pattern: "for February 2026" / "in January 2025"
    month_year_match = re.search(r'\b(?:for|in)\s+(january|february|march|april|may|june|july|august|september|october|november|december)\s+(\d{4})\b', query)
    if month_year_match:
    ...
    # Pattern: "N days/day back/behind/ago" and variants like "2 days back"
    days_back_match = re.search(r'\b(\d+)\s+days?\s+(?:back|behind|ago)\b', query)
    if days_back_match:
        days = int(days_back_match.group(1))
        target_start = day_start - timedelta(days=days)
        target_end = day_end - timedelta(days=days)
        date_filters['days_ago'] = days
        date_filters['start_date'] = target_start
        date_filters['end_date'] = target_end
        date_filters['type'] = 'days_range'

    # Pattern: "a day back" / "one day ago"
    if re.search(r'\b(?:a|an|one)\s+day\s+(?:back|behind|ago)\b', query):
        target_start = day_start - timedelta(days=1)
        target_end = day_end - timedelta(days=1)
        date_filters['days_ago'] = 1
        date_filters['start_date'] = target_start
        date_filters['end_date'] = target_end
        date_filters['type'] = 'days_range'
    ...
    # Pattern: "a week back" / "one week ago" (specific day one week ago)
    if re.search(r'\b(?:a|an|one)\s+week\s+(?:back|behind|ago)\b', query):
        target_start = day_start - timedelta(days=7)
        target_end = day_end - timedelta(days=7)
        date_filters['days_ago'] = 7
        date_filters['start_date'] = target_start
        date_filters['end_date'] = target_end
        date_filters['type'] = 'days_range'
...
def extract_text_filters(query: str) -> Dict[str, Any]:
    ...
    text_filters = {}
    query_for_name = query.strip().rstrip('?.!')
    ...
    of_match = re.search(r'of\s+([a-z][a-z0-9\s&-\.\']+?)(?:\s+(?:last|this|next|in|from|transactions?|customers?|data|records|month|week|day|year)|\s*$)', query_for_name)
    if of_match:
    ...
    # Pattern 0a: Extract merchant/business name from "for [Name]" in transaction-like queries
    # Matches: "last transaction for chicken republic", "failed transactions for KFC"
    if any(word in query for word in ['transaction', 'transcation', 'transcations', 'payment', 'debit', 'credit', 'balance']):
        for_match = re.search(r'for\s+([a-z][a-z0-9\s&-\.\']+?)(?:\s+(?:last|this|next|in|from|transactions?|transaction|customers?|data|records|month|week|day|year)|\s*$)', query_for_name)
        if for_match:
            merchant_name = for_match.group(1).strip()
            merchant_name = re.sub(r'\s+', ' ', merchant_name)
            merchant_name_lower = merchant_name.lower()
            invalid_targets = temporal_stopwords.union({'me', 'all', 'some', 'each', 'every'})
            if merchant_name_lower and merchant_name_lower not in invalid_targets and 'for each' not in query:
                text_filters['merchant_name'] = merchant_name.title()

    # Pattern 0c: Last/latest transaction intent
    if re.search(r'\b(last|latest)\s+trans(?:action|cation)s?\b', query):
        text_filters['last_transaction'] = True
    ...
    # Use word boundaries and prefer longer/specific statuses first
    status_patterns = [
        (r'\bsuccessful\b', 'SUCCESSFUL'),
        (r'\bsuccess\b', 'SUCCESSFUL'),
    ...
    for pattern, canonical_status in status_patterns:
        if re.search(pattern, query):
            text_filters['status'] = canonical_status
            break
...
def extract_rank_filters(query: str) -> Dict[str, Any]:
    """Extract top/bottom-N style rank filters from query."""
    ...
    rank_filters['direction'] = direction
    rank_filters['limit'] = limit
    if metric_phrase:
        rank_filters['metric_phrase'] = metric_phrase
